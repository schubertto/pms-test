...
try:
    ...
    # 5.3. 스프린트 데이터 주입 (요청에 따라 3단계 구조로 재구성 및 프로젝트 연결)
    print("[작업] 스프린트(pms_sprints) 초기 3단계 데이터를 등록합니다...")
    sprints = [
        ("스프린트 1: 기획 및 API 설계", "2026-06-01", "2026-06-15", False, project_id),
        ("스프린트 2: UI 디자인 및 백엔드 개발", "2026-06-16", "2026-06-30", True, project_id), # 오늘(6/26) 속한 스프린트 활성화
        ("스프린트 3: 프론트 연동 및 QA", "2026-07-01", "2026-07-31", False, project_id) # 7/31까지로 일정 연장
    ]
    for s in sprints:
        cursor.execute("""
    ...
    # [스프린트 1 업무 추가]
    # 1.1 요구사항 정의 및 화면 설계 (팀원1) - 부모 작업이 없는 최상위 작업
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("요구사항 정의 및 화면 설계", "프로젝트의 전반적인 기능 정의서와 와이어프레임 설계", "1.1", "완료", 3, member_map["팀원1"], sprint_map[1], "2026-06-01", "2026-06-07", 100))
    t1_1_id = cursor.fetchone()[0]

    # 1.2 DB 스키마 설계 및 생성 (팀원2) - 부모: 1.1 요구사항 정의
    cursor.execute("""
    ...
    """, ("DB 스키마 설계 및 테이블 생성", "Neon DB를 연동하기 위한 관계형 테이블 설계", "1.2", "완료", 3, member_map["팀원2"], sprint_map[1], "2026-06-08", "2026-06-12", 100, t1_1_id))
    t1_2_id = cursor.fetchone()[0]

    # 1.3 API 명세서 작성 (팀원4) - 부모: 1.1 요구사항 정의
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress, parent_id)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("API 명세서 및 Mock 데이터 생성", "백엔드와 프론트엔드가 공유할 REST API 설계 문서화", "1.3", "완료", 2, member_map["팀원4"], sprint_map[1], "2026-06-10", "2026-06-15", 100, t1_1_id))
    t1_3_id = cursor.fetchone()[0]

    # [스프린트 2 업무 추가]
    # 2.1 UI/UX 디자인 시안 제작 (팀원5) - 스프린트 2 기동
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("UI/UX 디자인 시안 제작", "피그마를 통한 대시보드 및 상세 페이지 비주얼 가이드 디자인", "2.1", "진행중", 3, member_map["팀원5"], sprint_map[2], "2026-06-16", "2026-06-22", 80))
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("웹 퍼블리싱 및 CSS 구축", "HTML 마크업 및 CSS 스타일 가이드 구현", "2.2", "예정", 2, member_map["팀원5"], sprint_map[2], "2026-06-23", "2026-06-30", 0, t2_1_id))
    ...
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress, dependency_id)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("백엔드 CRUD API 개발", "Neon DB와 연동하여 작업 추가/수정/삭제를 처리하는 API 작성", "2.3", "진행중", 4, member_map["팀원2"], sprint_map[2], "2026-06-16", "2026-06-25", 90, t1_3_id))
    ...
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress, dependency_id)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("프론트엔드 기본 레이아웃 구현", "사이드바, 네비게이션바 및 페이지 컨테이너 뼈대 개발", "2.4", "진행중", 3, member_map["팀원3"], sprint_map[2], "2026-06-18", "2026-06-26", 50, t1_1_id))
    ...
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress, dependency_id)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("API 연동 및 동적 차트 개발", "프론트엔드와 백엔드 API 연동 및 실시간 Plotly 그래프 렌더링", "3.1", "예정", 4, member_map["팀원3"], sprint_map[3], "2026-07-01", "2026-07-10", 0, t2_3_id))
    ...
    cursor.execute("""
        INSERT INTO pms_tasks (title, description, wbs_code, status, weight, assignee_id, sprint_id, start_date, end_date, progress, dependency_id)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id;
    """, ("통합 테스트 및 버그 수정 QA", "최종 빌드 후 기능별 단위 테스트 및 성능 QA 수행", "3.2", "예정", 3, member_map["팀원1"], sprint_map[3], "2026-07-11", "2026-07-15", 0, t3_1_id))
